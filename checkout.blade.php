...
<main class="pt-90">
    ...
    <section class="shop-checkout container">
      ...
      <form name="checkout-form" id="checkout-form" action="{{ route('cart.place.an.order') }}" method="POST">
  @csrf
    ...
    <div class="my-account__address-list p-4 border rounded-3 bg-white shadow-sm">
      <div class="my-account__address-list-item">
        <div class="my-account__address-item__detail" style="font-size: 15px; color: #333;">
          ...
          <p class="mb-1"><strong>State/Country:</strong> {{ $address->state }}, {{ $address->country }}</p>
          <p class="mb-1"><strong>Pincode:</strong> {{ $address->zip }}</p>
          <hr class="my-3" />
            ...
            <table class="checkout-totals">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-end">${{ Session::get('discounts')['discount'] }}</td>
                </tr>
                <tr>
                  <th>Subtotal After Discount</th>
                  <td class="text-end">${{ Session::get('discounts')['subtotal'] }}</td>
                </tr>
                <tr>
                  <th>Shipping</th>
                ...
                <tr>
                  <th>VAT</th>
                  <td class="text-end">${{ Session::get('discounts')['tax'] }}</td>
                </tr>
                <tr>
                  <th>Total</th>
                  <td class="text-end">${{ Session::get('discounts')['total'] }}</td>
                </tr>
              </tbody>
            </table>
          @else
...
document.getElementById('checkout-form').addEventListener('submit', function (e) {
    ...
    if (selected === 'stripe') {
        ...
        this.action = "{{ route('stripe.checkout') }}"; // Redirect to Stripe route
